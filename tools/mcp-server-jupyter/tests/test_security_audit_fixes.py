...
"""
        ...
        assert not Path(
            "/tmp/hacked.txt"
        ).exists(), "CRITICAL: Code injection executed!"
...
async def test_reaper_fratricide_prevention(tmp_path):
    ...
    try:
        ...
        km = session_a["km"]
        kernel_process = (
            km.provisioner.process
            if hasattr(km, "provisioner") and km.provisioner
            else getattr(km, "kernel", None)
        )
        ...
        assert psutil.pid_exists(
            kernel_a_pid
        ), "FRATRICIDE DETECTED! Kernel A was killed by Server B"

        # Verify Kernel A is still responsive
        exec_id = await server_a.execute_cell_async(
            str(nb_a), -1, "print('Still alive')"
        )
        ...
        assert status["status"] in [
            ...
            "busy",
            "running",
        ], "Kernel A should still be responsive"
    finally:
        # Cleanup
...
async def test_reaper_kills_orphaned_kernels(tmp_path):
    ...
    try:
        ...
        (server_a.persistence_dir / f"session_{hash(str(nb_path))}.json")
...
async def test_is_kernel_busy(tmp_path):
    ...
    try:
        ...
        assert not manager.is_kernel_busy(
            str(nb_path)
        ), f"Kernel should not be busy after completion. Status: {status}"
    finally:
        # Cleanup
        await manager.stop_kernel(str(nb_path))
...
"""
        ...
        assert (
            successful_polls < poll_attempts
        ), "Dashboard should skip most polls when kernel is busy"
        assert (
            successful_polls <= 1
        ), f"Expected 0-1 successful polls, got {successful_polls}"
        ...
        assert not manager.is_kernel_busy(
            str(nb_path)
        ), f"Kernel should not be busy now. Status: {status}"
    finally:
        # Cleanup
        await manager.stop_kernel(str(nb_path))
...
async def test_server_pid_tracking(tmp_path):
    ...
    try:
        ...
        assert (
            session_data["server_pid"] == os.getpid()
        ), "server_pid should match current process"
    finally:
        # Cleanup
        await manager.stop_kernel(str(nb_path))
