...
async def test_sql_injection_triple_quote_breakout(tmp_path):
    ...
    Attack vector: Inject """ to break out of triple-quoted string
...
"""
    ...
    malicious_query = 'SELECT * FROM df""" + __import__("os").system("echo HACKED > /tmp/hacked.txt") + """'
...
"""
    ...
    query_with_quotes = 'SELECT * FROM df WHERE name = "Alice"""'
...
async def test_reaper_fratricide_prevention(tmp_path):
    ...
    kernel_a_pid = session_a['km'].kernel.pid
...
async def test_is_kernel_busy(tmp_path):
    ...
    assert not manager.is_kernel_busy(str(nb_path)), "Kernel should not be busy after completion"
...
"""
    ...
    assert not manager.is_kernel_busy(str(nb_path)), "Kernel should not be busy now"
