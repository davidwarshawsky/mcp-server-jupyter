...
class TestStrictModeContract:
    ...
    def test_strict_mode_blocks_wildcard_allowlist(self):
        ...
        with patch.dict(
            os.environ, {"MCP_STRICT_MODE": "1", "MCP_PACKAGE_ALLOWLIST": "*"}
        ):
    ...
    def test_strict_mode_allows_explicit_allowlist(self):
        ...
        with patch.dict(
            os.environ,
            {"MCP_STRICT_MODE": "1", "MCP_PACKAGE_ALLOWLIST": "pandas,numpy"},
        ):
    ...
    def test_non_strict_mode_allows_wildcard(self):
        ...
        with patch.dict(
            os.environ, {"MCP_STRICT_MODE": "0", "MCP_PACKAGE_ALLOWLIST": "*"}
        ):
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(returncode=0, stdout="Success")
...
class TestPIDValidationContract:
    ...
    def test_legacy_sessions_without_uuid_skipped(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Capture logs
            with patch.object(
                logging.getLogger("src.kernel_state"), "warning"
            ) as mock_warning:
                state_manager.reconcile_zombies()
                ...
                assert any(
                    "has no kernel_uuid" in str(call).lower()
                    or "legacy" in str(call).lower()
                    for call in warning_calls
                ), f"Expected warning about missing kernel_uuid, got: {warning_calls}"
...
class TestPerformanceContract:
    ...
    def test_config_load_completes_under_100ms(self):
        ...
        assert (
            elapsed < 0.1
        ), f"Config loading took {elapsed*1000:.1f}ms (expected <100ms)"
