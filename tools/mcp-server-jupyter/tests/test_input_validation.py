...
from src.models import (
    StartKernelArgs, StopKernelArgs, RunCellArgs, InstallPackageArgs,
    GetVariableInfoArgs, InspectVariableArgs, SetWorkingDirectoryArgs,
    QueryDataframesArgs, SwitchKernelEnvironmentArgs
)
...
class TestShellInjectionPrevention:
    ...
    def test_install_package_blocks_semicolons(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            InstallPackageArgs(notebook_path="/home/test.ipynb", package="pandas; rm -rf /")
    ...
    def test_install_package_blocks_pipes(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            InstallPackageArgs(notebook_path="/home/test.ipynb", package="pandas | cat /etc/passwd")
    ...
    def test_install_package_blocks_backticks(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            InstallPackageArgs(notebook_path="/home/test.ipynb", package="pandas `whoami`")
    ...
    def test_install_package_blocks_ampersands(self):
        ...
        assert "Shell metacharacters not allowed" in str(exc_info.value) or "Command chaining not allowed" in str(exc_info.value)
    
    def test_set_working_directory_blocks_shell_chars(self):
        """SetWorkingDirectoryArgs should reject shell metacharacters."""
        with pytest.raises(ValidationError) as exc_info:
            SetWorkingDirectoryArgs(notebook_path="/home/test.ipynb", path="/home/user; rm -rf /")
    ...
    def test_switch_kernel_environment_blocks_shell_chars(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SwitchKernelEnvironmentArgs(notebook_path="/home/test.ipynb", venv_path="/venv`whoami`")
...
class TestCodeInjectionPrevention:
    ...
    def test_run_cell_enforces_max_code_length(self):
        ...
        large_code = "x = 1\n" * 50000  # ~300KB
        with pytest.raises(ValidationError) as exc_info:
            RunCellArgs(notebook_path="/home/test.ipynb", index=0, code_override=large_code)
        
        assert "too long" in str(exc_info.value).lower() or "100000 characters" in str(exc_info.value)
    ...
    def test_query_dataframes_blocks_dangerous_sql(self):
        ...
        for sql in dangerous_queries:
            ...
            assert "dangerous" in str(exc_info.value).lower() and "sql" in str(exc_info.value).lower()
    ...
    def test_query_dataframes_enforces_max_length(self):
        ...
        large_query = "SELECT * FROM table WHERE id IN (" + ",".join(str(i) for i in range(15000)) + ")"
        with pytest.raises(ValidationError) as exc_info:
            QueryDataframesArgs(notebook_path="/home/test.ipynb", sql_query=large_query)
        
        assert "too long" in str(exc_info.value).lower() or "50000 characters" in str(exc_info.value)
...
class TestPythonIdentifierValidation:
    ...
    def test_get_variable_info_validates_identifier(self):
        ...
        for name in invalid_names:
            ...
            assert "Invalid Python identifier" in str(exc_info.value) or "cannot be empty" in str(exc_info.value)
    
    def test_inspect_variable_validates_identifier(self):
        """InspectVariableArgs should validate Python identifiers."""
        with pytest.raises(ValidationError) as exc_info:
            InspectVariableArgs(notebook_path="/home/test.ipynb", variable_name="my-var")
...
class TestValidInputsAccepted:
    ...
    def test_query_dataframes_valid_inputs(self):
        ...
        for query in valid_queries:
            args = QueryDataframesArgs(notebook_path="/home/test.ipynb", sql_query=query)
...
class TestDockerImageValidation:
    ...
    def test_start_kernel_blocks_shell_chars_in_docker_image(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            StartKernelArgs(notebook_path="/home/test.ipynb", docker_image="python:3.10; rm -rf /")
