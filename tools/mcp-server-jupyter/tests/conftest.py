            ...
            if "ipykernel_launcher" in line or "jupyter-kernel" in line:
                ...
                if len(parts) > 1:
                    try:
                        pid = int(parts[1])
                        subprocess.run(
                            ["kill", "-9", str(pid)], capture_output=True, timeout=1
                        )
...
def pytest_pyfunc_call(pyfuncitem):
    """Run coroutine test functions under an asyncio loop when pytest-asyncio is not installed.

    This provides a minimal compatibility shim so tests written with "async def" will
    execute even if pytest-asyncio isn't present in the environment. If a proper
    pytest-asyncio plugin is available it will take precedence.
    """
    import asyncio
    import inspect
    ...
    if inspect.iscoroutinefunction(pyfuncitem.obj):
        ...
        try:
            ...
            coro = pyfuncitem.obj(**allowed_kwargs)
    ...
    async def mock_get_iopub_msg(timeout=None):
        ...
        try:
            msg = await asyncio.wait_for(
                mock_kc._iopub_channel.get(), timeout=timeout if timeout else None
            )
            return msg
...
@pytest.fixture(autouse=True)
def auto_cleanup_session_managers(monkeypatch):
    """
    ...
    This fixture is intentionally synchronous to be compatible with both synchronous
    and asynchronous tests. Cleanup is performed in a best-effort manner: synchronous
    shutdown calls are preferred; if an async coroutine is returned, it is scheduled
    on the running event loop when possible (fire-and-forget) or executed via
    asyncio.run() when no loop is active.
    """
    try:
        from src.session import SessionManager
        # Only treat manager as available if it appears to be a real class defined by the project
        manager_available = isinstance(SessionManager, type) and getattr(SessionManager, "__module__", "").startswith("src")
    except ModuleNotFoundError:
        # If core components aren't importable in this test environment, skip tracking.
        manager_available = False

    # Track all SessionManager instances created during this test
    tracked_managers = []
    original_init = getattr(SessionManager, "__init__", None) if manager_available else None
    ...
    # Monkey-patch the __init__ to track instances if available and safe to patch
    if manager_available:
        try:
            monkeypatch.setattr(SessionManager, "__init__", tracking_init)
        except TypeError:
            # Some test shims may inject placeholder objects (e.g., SessionManager=object);
            # in that case, avoid patching to prevent immutable-type errors.
            manager_available = False
        ...
        try:
            ...
            for nb_path in list(manager.sessions.keys()):
                try:
                    ...
                    # Close kernel client channels (sync preferred; schedule coroutine if returned)
                    if "kc" in session and hasattr(session["kc"], "stop_channels"):
                        try:
                            result = session["kc"].stop_channels()
                            if asyncio.iscoroutine(result):
                                # If an event loop is running, schedule it fire-and-forget
                                try:
                    ...
                    # Stop kernel manager (prefer sync; schedule coroutine if returned)
                    if "km" in session and hasattr(session["km"], "shutdown_kernel"):
                        try:
                            res = session["km"].shutdown_kernel(now=True)
    ...
    # Best-effort: if there is a running event loop, let it schedule cleanup tasks briefly
    try:
        loop = asyncio.get_running_loop()
        # Schedule a short no-op to yield control (non-blocking)
        loop.call_soon_threadsafe(lambda: None)
    except RuntimeError:
        # No loop running; nothing else to do
        pass
...
def mock_async_kernel_manager():
    ...
    mock_kc.get_shell_msg = AsyncMock(
        return_value={
            "content": {"status": "ok"},
