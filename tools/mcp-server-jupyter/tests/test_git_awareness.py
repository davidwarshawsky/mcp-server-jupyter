...
from src.git_tools import save_notebook_clean, create_agent_branch
from src.asset_manager import (
    prune_unused_assets,
    get_assets_summary,
    ensure_assets_gitignored,
)
...
class TestAssetManagement:
    ...
    def test_prune_unused_assets_removes_orphans(self, tmp_path):
        ...
        (assets_dir / "asset_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.png").write_bytes(
            b"orphan"
        )
...
class TestGetNotebookOutline:
    ...
    def test_outline_includes_cell_ids(self, tmp_path):
        ...
        nb_path = tmp_path / "test.ipynb"
        create_notebook(
            str(nb_path), initial_cells=[{"type": "code", "content": "x = 1"}]
        )
