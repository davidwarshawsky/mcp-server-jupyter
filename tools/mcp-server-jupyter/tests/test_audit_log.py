...
class TestAuditLogger:
    ...
    def test_volume_limit_warning(self, mock_logger):
        ...
        assert (
            len(warning_calls) > 0
        ), f"Expected volume warning, got calls: {mock_logger.warning.call_args_list}"
    ...
    def test_error_logs_never_dropped(self, mock_logger):
        ...
        initial_error_count = len(
            [c for c in mock_logger.error.call_args_list if "AUDIT:" in str(c)]
        )
    ...
    def test_kernel_error_events_never_dropped(self, mock_logger):
        ...
        for i in range(100):
            logger.log_kernel_event(
                event_type="heartbeat", kernel_id="kernel-1", status="success"
            )
    ...
    def test_volume_counter_resets_hourly(self):
        ...
        with patch("src.audit_log.logger"):
            logger.log_tool_execution(tool="test", status="success", duration_ms=1.0)
...
class TestAuditToolDecorator:
    ...
    async def test_metadata_extraction(self):
        ...
        with patch.object(audit_logger, "log_tool_execution") as mock_log:
            await test_func(kernel_id="kernel-123", cell_index=5, other="ignored")
...
class TestLogFormat:
    ...
    def test_json_format(self, mock_logger):
        ...
        logger.log_tool_execution(
            tool="test", status="success", duration_ms=100.0, trace_id="abc"
        )
    ...
    def test_required_fields(self, mock_logger):
        ...
        logger.log_tool_execution(tool="test", status="success", duration_ms=50.0)
    ...
    def test_duration_precision(self, mock_logger):
        ...
        logger.log_tool_execution(tool="test", status="success", duration_ms=123.456789)
...
class TestPerformance:
    ...
    def test_logging_overhead_minimal(self):
        ...
        with patch("src.audit_log.logger"):
            for _ in range(1000):
                logger.log_tool_execution(
                    tool="test", status="success", duration_ms=1.0
                )
