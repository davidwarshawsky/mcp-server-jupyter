...
def assert_cell_equal(cell1: Dict[str, Any], cell2: Dict[str, Any], ignore_outputs: bool = True) -> None:
    """
...
def assert_cell_type(nb_path: str, cell_index: int, expected_type: str) -> None:
    ...
    assert 0 <= cell_index < len(nb.cells), \
...
def assert_cell_has_tag(nb_path: str, cell_index: int, tag: str) -> None:
    ...
    assert tag in tags, \
        f"Tag '{tag}' not found in cell {cell_index}. Tags: {tags}"
...
def assert_metadata_contains(nb_path: str, key: str, expected_value: Any = None) -> None:
    """
