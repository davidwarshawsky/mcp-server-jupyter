...
async def simulate_human_client(uri, token=None):
    ...
    try:
        ...
        print(
            "❌ Connection refused. Is the server running? (python -m src.main --transport websocket)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--token", type=str, default=None, help="Auth token for WebSocket connection"
    )
    ...
    print(
        "Run the server first in another terminal: python -m src.main --transport websocket --port 3000 --host 0.0.0.0"
    )
