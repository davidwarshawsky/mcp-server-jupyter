...
def test_text_offloading_large_output():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"✓ Stub length: {len(llm_summary)} chars (original: {len(large_text)} chars)"
        )
...
def test_small_output_not_offloaded():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        outputs = [{"output_type": "stream", "name": "stdout", "text": small_text}]
...
def test_execute_result_offloading():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        large_array = "\n".join(
            [f"[{i}, {i+1}, {i+2}, ...]" for i in range(0, 1000, 3)]
        )
        ...
        print(
            f"✓ Execute result offloaded: {len(large_array)} chars → {asset_files[0].name}"
        )
...
def test_asset_reference_tracking():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        output.metadata = {"mcp_asset": {"path": f"{assets_dir}/{asset_name}"}}
        cell.outputs = [output]
        ...
        assert (
            asset_name in referenced
        ), f"Expected {asset_name} to be detected, got: {referenced}"
...
def test_prune_unused_assets():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            assets_dir / referenced_name
        ).exists(), "Referenced asset should be kept"
        assert not (
            assets_dir / orphaned_name
        ).exists(), "Orphaned asset should be deleted"
...
def test_read_asset_tool():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        content = "\n".join(
            [f"Line {i}: Some important log message" for i in range(1, 101)]
        )
...
def test_integration_full_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        large_output = "\n".join(
            [f"Epoch {i}: Loss {1.0 / (i + 1):.6f}" for i in range(1, 1001)]
        )
        ...
        print("  Step 2: Notebook created with stubbed output")
...
def test_regex_captures_diverse_assets():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "asset_abc123456789.json" in refs
        ), "Failed to catch JSON asset with short hash"
        assert (
            "asset_11112222333344445555666677778888.svg" in refs
        ), "Failed to catch SVG asset"
        assert (
            "text_abcdef123456abcdef123456abcdef12.txt" in refs
        ), "Failed to catch Text asset"
...
def test_prune_ignores_user_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Run GC
        prune_unused_assets(str(nb_path), dry_run=False)

        # Verify
        assert not (
            assets_dir / "text_12345678901234567890123456789012.txt"
        ).exists(), "System file was not deleted"
        assert (
            assets_dir / "my_important_model.pkl"
        ).exists(), "User file was incorrectly deleted"
