...
class MCPWebSocketHarness:
    ...
    async def start(self):
        ...
        try:
            ...
            self.ws = await asyncio.wait_for(
                websockets.connect(self.base_url, subprotocols=["mcp"]), timeout=10.0
            )
    ...
    async def _drain_process_output(self, timeout: float = 1.0):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            if self.proc.stderr:
                try:
                    line = await asyncio.wait_for(
                        self.proc.stderr.readline(), timeout=0.1
                    )
                    if line:
    ...
    async def send_request(self, method, params=None):
        ...
        req = {
            "jsonrpc": "2.0",
            "id": req_id,
            "method": "tools/call",  # MCP standard tool call
    ...
    try:
        ...
        if "result" in resp:
            ...
            if content:
                ...
                assert (
                    "created" in content[0]["text"] or "Created" in content[0]["text"]
                )
