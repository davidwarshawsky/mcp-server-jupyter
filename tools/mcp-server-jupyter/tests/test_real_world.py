...
"""
    ...
    has_plot_output = (
        ...
        or "Plot saved" in output_text
        or "test_plot.png" in output_text
    )
    ...
    print("+ Asset extraction test passed")
    print(f"  - PNG saved to: {png_file}")
    print(
        f"  - Provenance: env={mcp_meta['kernel_env_name']}, time={mcp_meta['execution_timestamp']}"
    )
...
"""
    ...
    assert inner_data["shape"] == [
        5,
        3,
    ], f"Shape should be [5, 3], got {inner_data['shape']}"
    ...
    missing_info = await real_session_manager.get_variable_info(
        str(nb_path), "nonexistent_var"
    )
    ...
    assert (
        "not found" in missing_data["error"].lower()
    ), "Error message should mention 'not found'"

    print("+ Variable inspection test passed")
    print(
        f"  - DataFrame: shape={inner_data['shape']}, columns={inner_data['columns']}"
    )
...
async def test_autoreload_enabled_on_startup(tmp_path, real_session_manager):
    ...
    exec_id2 = await real_session_manager.execute_cell_async(
        str(nb_path), 1, check_code
    )
...
"""
    ...
    if assets_dir.exists():
        ...
        print("+ Multiple asset types test passed")
