...
from src.main import ConnectionManager, get_server_status, connection_manager
...
async def test_server_heartbeat_shutdown():
    ...
    """
    from src.main import ConnectionManager, IDLE_TIMEOUT
    ...
    with patch("time.time") as mock_time:
        ...
        ), "Time advancement should exceed idle timeout"
        assert (
            len(cm.active_connections) == 0
        ), "No active connections, should trigger shutdown"

        print(
            f"✓ Idle timeout condition met after {idle_duration}s (threshold: {IDLE_TIMEOUT}s)"
        )
