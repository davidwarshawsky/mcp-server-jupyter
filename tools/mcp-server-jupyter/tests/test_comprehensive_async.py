...
async def test_async_error_handling(session_manager_fixture, temp_notebook):
    ...
    has_error_output = "Error" in str(
        final_status.get("output", "")
    ) or "Traceback" in str(final_status.get("output", ""))

    assert (
        is_error_status or has_error_output
    ), f"Execution failed to report error. Status: {final_status['status']}, Output: {final_status.get('output')}"
    ...
    has_error = (
        "Intentional Error" in output_str
        or "ValueError" in output_str
        or final_status.get("intermediate_outputs_count", 0) > 0
    )
