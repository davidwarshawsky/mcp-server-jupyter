...
class TestEnvironmentActivation:
    ...
    def test_conda_activation_linux(self, mock_os, mock_path):
        ...
        # Execute
        env = _get_activated_env_vars(
            "/opt/conda/envs/myenv", "/opt/conda/envs/myenv/bin/python"
        )
    ...
    def test_conda_activation_windows(self, mock_os, mock_path):
        ...
        def path_truediv(other):
            ...
            if other == "Library":
                ...
                m = MagicMock()
                m.__truediv__.side_effect = lambda x: (
                    lib_bin if x == "bin" else MagicMock()
                )
                return m
            return MagicMock()
        ...
        # Execute
        env = _get_activated_env_vars(
            r"C:\conda\envs\myenv", r"C:\conda\envs\myenv\python.exe"
        )
...
async def test_install_package_invalidates_caches():
    ...
    manager.sessions["/tmp/nb.ipynb"] = {"km": MagicMock(), "kc": MagicMock()}
    # Mock kernel command
    ...
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        assert (
            "importlib.invalidate_caches()" in code_sent
        ), f"Cache invalidation not found in: {code_sent}"
