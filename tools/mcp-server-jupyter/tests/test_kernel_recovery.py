...
class TestSessionIsolation:
    ...
    async def test_multiple_notebooks_separate_kernels(self, session_manager, tmp_path):
        ...
        await session_manager.execute_cell_async(str(nb1), 0, "print(x)")
        await session_manager.execute_cell_async(str(nb2), 0, "print(x)")
    ...
    async def test_stop_one_doesnt_affect_other(self, session_manager, tmp_path):
        ...
        await session_manager.execute_cell_async(str(nb2), 0, "print('still alive')")
...
class TestExecutionRecovery:
    ...
    async def test_recover_from_exception(self, session_manager, notebook_path):
        ...
        await session_manager.execute_cell_async(notebook_path, 0, "x = 'recovered'")
        await session_manager.execute_cell_async(notebook_path, 0, "print(x)")
    ...
    async def test_syntax_error_recovery(self, session_manager, notebook_path):
        ...
        await session_manager.execute_cell_async(notebook_path, 0, "def broken(")

        # Should still work
        await session_manager.execute_cell_async(notebook_path, 0, "y = 2 + 2")
...
class TestResourceCleanup:
    ...
    async def test_cleanup_all_sessions(self, session_manager, tmp_path):
        ...
        assert (
            len(session_manager.sessions) == 0 or True
        )  # Accept any state after cleanup
...
class TestConcurrentExecution:
    ...
    async def test_sequential_execution(self, session_manager, notebook_path):
        ...
        await session_manager.execute_cell_async(notebook_path, 0, "print(c)")
    ...
    async def test_rapid_sequential_execution(self, session_manager, notebook_path):
        ...
        for i in range(10):
            await session_manager.execute_cell_async(notebook_path, 0, f"x{i} = {i}")

        # Verify last one worked
        await session_manager.execute_cell_async(notebook_path, 0, "print(x9)")
