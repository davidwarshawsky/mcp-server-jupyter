...
def test_compress_traceback_removes_library_frames():
    ...
    assert any(
        "notebook.ipynb" in line for line in compressed
    ), "User code frame should be kept"
    assert any(
        "Internal Library Frames" in line for line in compressed
    ), "Library frames should be replaced with placeholder"
    assert any(
        "ValueError" in line for line in compressed
    ), "Final error message should be kept"

    # Verify compression: original 14 lines -> ~5-6 lines
    assert (
        len(compressed) < 10
    ), f"Traceback should be compressed, got {len(compressed)} lines"

    # Verify no duplicate placeholders
    placeholder_count = sum(
        1 for line in compressed if "Internal Library Frames" in line
    )
    assert (
        placeholder_count == 1
    ), "Should have only one placeholder for consecutive library frames"
...
async def test_linearity_guard_detects_out_of_order(tmp_path):
    ...
    assert (
        "INTEGRITY WARNING" in output or "hidden state" in output.lower()
...
async def test_linearity_guard_allows_forward_execution(tmp_path):
    ...
    assert "INTEGRITY WARNING" not in output, "Should not warn on forward execution"
