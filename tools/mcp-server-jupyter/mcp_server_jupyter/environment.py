...
def detect_environment_type(python_path: str) -> Tuple[str, str]:
    ...
    # Check for conda
    if (
        "conda" in str(python_path_obj).lower()
        or "anaconda" in str(python_path_obj).lower()
        or "miniconda" in str(python_path_obj).lower()
    ):
    ...
    # Check for poetry
    if (
        "poetry" in str(python_path_obj).lower()
        or "pypoetry" in str(python_path_obj).lower()
    ):
        return ("poetry", python_path_obj.parent.parent.name)

    # Check for pipenv
    if "pipenv" in str(python_path_obj).lower() or ".virtualenvs" in str(
        python_path_obj
    ):
...
def validate_python_executable(python_path: str) -> Dict[str, Any]:
    ...
    if not version_info["valid"]:
        result["error"] = (
            f"Failed to execute Python: {version_info.get('error', 'unknown')}"
        )
        return result
...
def install_package(
    ...
    if allowlist_str.strip() == "*":
        ...
        if base_package.lower() not in allowed_packages:
            return (
                False,
                f"Package '{base_package}' is blocked by policy. To allow this package, ask your admin to add '{base_package}' to MCP_PACKAGE_ALLOWLIST in the server config.",
            )
...
def get_activated_env_vars(venv_path: str, python_exe: str) -> Optional[Dict[str, str]]:
    ...
    if os.name == "nt":
        ...
        paths_to_add = [
            str(p) for p in [scripts, lib_bin, lib_usr_bin, venv_path_obj] if p.exists()
        ]
