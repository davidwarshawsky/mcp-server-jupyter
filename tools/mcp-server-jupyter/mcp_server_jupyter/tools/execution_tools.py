...
def register_execution_tools(mcp, session_manager):
    ...
    async def run_cell_async(
        ...
        # 0. Respect sync policy: If not forced, check for sync needed and instruct client
        if not force:
            try:
                ...
                try:
                    sync_obj = json.loads(sync_resp)
                except Exception:
                    sync_obj = {}
                if sync_obj.get("sync_needed"):
                    return json.dumps(
                        {
                            "error": "sync_needed",
                            "recommendation": "sync_state_from_disk",
                            "changed_cells": sync_obj.get("changed_cells", []),
                        },
                        indent=2,
                    )
            except Exception:
                # If sync check fails, prefer to continue rather than block execution
                pass
        ...
        if is_busy:
            ...
            if session:
                if session["queued_executions"]:
                    result["reason"] = (
                        f"{len(session['queued_executions'])} executions queued"
                    )
                else:
