...
def migrate_notebook_to_cell_ids(notebook_path: str) -> str:
    ...
    try:
        ...
        if was_modified:
            ...
            if needs_upgrade:
                msg_parts.append(
                    f"Upgraded nbformat from {original_version[0]}.{original_version[1]} to 4.5"
                )
            if cells_updated > 0:
...
) -> Optional[Tuple[int, nbformat.NotebookNode]]:
    """
...
def validate_cell_id_at_index(
    nb: nbformat.NotebookNode, index: int, expected_id: str
) -> bool:
    """
...
def edit_cell_by_id(
    ...
    if not result:
        raise StaleStateError(
            f"Cell ID {cell_id} not found. Notebook may have been modified."
        )
...
def delete_cell_by_id(
    notebook_path: str, cell_id: str, expected_index: Optional[int] = None
) -> str:
    """
...
def insert_cell_by_id(
    ...
    if after_cell_id is None:
        ...
        if not result:
            raise StaleStateError(
                f"Cell ID {after_cell_id} not found. Notebook may have been modified."
            )
