...
def validate_notebook_path(notebook_path: str, must_exist: bool = False) -> Path:
    ...
    if not isinstance(notebook_path, str):
        raise ValidationError(
            f"Notebook path must be a string, got {type(notebook_path).__name__}"
        )
    ...
    if path.suffix.lower() != ".ipynb":
        raise ValidationError(
            f"Notebook must have .ipynb extension, got '{path.suffix}'"
        )
...
def validate_cell_index(
    index: int, total_cells: int, allow_negative: bool = True
) -> int:
    """
    ...
    if not isinstance(index, int):
        raise ValidationError(
            f"Cell index must be an integer, got {type(index).__name__}"
        )
...
def validate_cell_type(cell_type: str) -> str:
    ...
    if not isinstance(cell_type, str):
        raise ValidationError(
            f"Cell type must be a string, got {type(cell_type).__name__}"
        )
...
def validate_cell_content(content: Any) -> str:
    ...
    if not isinstance(content, str):
        try:
            ...
            raise ValidationError(
                f"Cell content must be a string, got {type(content).__name__}"
            )
...
def validate_initial_cells(initial_cells: Optional[list]) -> list:
    ...
    for i, cell_spec in enumerate(initial_cells):
        ...
        validated.append({"type": validated_type, "content": validated_content})
...
def validate_kernel_name(kernel_name: str) -> str:
    ...
    if not isinstance(kernel_name, str):
        raise ValidationError(
            f"Kernel name must be a string, got {type(kernel_name).__name__}"
        )
...
def validate_venv_path(venv_path: Optional[str]) -> Optional[Path]:
    ...
    if not isinstance(venv_path, str):
        raise ValidationError(
            f"venv_path must be a string, got {type(venv_path).__name__}"
        )
    ...
    if not path.is_dir():
        raise ValidationError(
            f"Virtual environment path is not a directory: {venv_path}"
        )
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            logging.getLogger(__name__).exception(
                "Unexpected error in async tool function"
            )
            return f"Error: Unexpected error - {e}"
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # 3. Execute
                result = (
                    await func(**validated_data.model_dump())
                    if asyncio.iscoroutinefunction(func)
                    else func(**validated_data.model_dump())
                )
