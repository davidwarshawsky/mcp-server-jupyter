...
def setup_git_filters(repo_path: str = ".") -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ["nbstripout", "--version"], capture_output=True, text=True, timeout=5
        )
        ...
        return (
            "Error: nbstripout command not found. Install with: pip install nbstripout"
        )
...
def create_agent_branch(repo_path: str = ".", branch_name: str = "") -> str:
    ...
    try:
        ...
        if repo.head.is_detached:
            return (
                "Error: Repository is in detached HEAD state. Checkout a branch first."
            )
...
def commit_agent_work(
    repo_path: str = ".", message: str = "", files: Optional[list] = None
) -> str:
    """
    ...
    try:
        ...
        files_to_add = []
        for file in files:
            # Consistent resolution: Always resolve to absolute path
            path_obj = Path(file)

            if path_obj.is_absolute():
                abs_path = path_obj.resolve()
            else:
                # If relative, assumes relative to CWD (standard CLI behavior)
                abs_path = (Path(os.getcwd()) / path_obj).resolve()

            # Verify the file is actually inside the repo
            try:
                abs_path.relative_to(repo_root)
            except ValueError:
                return f"Error: File {file} is outside the repository root {repo_root}"

            if not abs_path.exists():
                return f"Error: File not found: {file}"

            files_to_add.append(str(abs_path))
        ...
        # GitPython expects paths relative to repo root, or absolute paths
