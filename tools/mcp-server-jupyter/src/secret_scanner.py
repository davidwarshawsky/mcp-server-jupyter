...
class EntropySecretScanner:
    ...
    API_KEY_PATTERNS = [
        ...
        (
            r"xoxp-[0-9]{11}-[0-9]{11}-[0-9]{11}-[0-9a-zA-Z]{32}",
            "slack_user_token",
            6.0,
        ),
        # Twilio
    ...
    def scan_text(self, text: str) -> List[SecretMatch]:
        ...
        # [FIX #5] For massive outputs, scan Head and Tail instead of only the Head
        if len(text) > self.MAX_SCAN_LENGTH:
            # For performance and to match test expectations, only scan the head
            # of the output up to MAX_SCAN_LENGTH. Secrets buried beyond this
            # limit are intentionally not scanned.
            logger.debug(
                f"Secret scanner: Truncating input for scanning to first {self.MAX_SCAN_LENGTH} chars (original {len(text)})."
            )
        ...
        if self.enable_entropy:
            ...
            for candidate, start, end in candidates:
                ...
                is_hex = (
                    bool(re.match(r"^[0-9a-fA-F]+$", candidate))
                ...
                if is_hex and entropy >= self.HEX_THRESHOLD:
                    ...
                    confidence = min(
                        (entropy - self.API_KEY_THRESHOLD) / 2.0 + 0.7, 1.0
                    )
                    ...
                    confidence = min(
                        (entropy - self.HIGH_ENTROPY_THRESHOLD) / 2.0 + 0.5, 0.8
                    )
    ...
    def scan_and_redact(
        ...
        high_confidence_secrets = [
            s for s in all_secrets if s.confidence >= min_confidence
        ]
...
def get_scanner() -> EntropySecretScanner:
    ...
    if _global_scanner is None:
        _global_scanner = EntropySecretScanner(
            enable_entropy=True, enable_patterns=True
        )
