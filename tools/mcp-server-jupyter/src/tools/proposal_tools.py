...
def save_proposals():
    ...
    try:
        ...
        with open(PROPOSAL_STORE_FILE, "w") as f:
            json.dump(
                {"store": PROPOSAL_STORE, "_history": list(PROPOSAL_HISTORY)},
                f,
                indent=2,
            )
    except Exception as e:
...
def register_proposal_tools(mcp):
    ...
    def notify_edit_result(
        ...
        """
        logger.info(
            f"Edit result for {notebook_path} (ID: {proposal_id}): {status} - {message}"
        )
        ...
        if proposal_id:
            try:
                ...
                existing.update(
                    {"status": status, "message": message, "updated_at": timestamp}
                )
        ...
        return json.dumps(
            {"status": "ack", "proposal_id": proposal_id, "timestamp": timestamp}
        )
...
def notify_edit_result(notebook_path: str, proposal_id: str, status: str, message: Optional[str] = None):
    """
    Backwards-compatible wrapper so other parts of the system (and tests)
    can import `notify_edit_result` from this module directly. Delegates
    to the same persistence logic used by the tool registered in
    ...
    if proposal_id:
        try:
            ...
            PROPOSAL_STORE[proposal_id] = {"status": status, "message": message, "updated_at": timestamp}

        try:
            save_proposal(proposal_id, PROPOSAL_STORE.get(proposal_id, {}))
        except Exception:
            save_proposals()
