...
class StartKernelArgs(SecureBaseModel):
    ...
    venv_path: Optional[str] = Field(default="", description="Path to Python virtual environment")
    docker_image: Optional[str] = Field(default="", description="Docker image name for sandboxed execution")
    timeout: int = Field(default=300, ge=10, le=3600, description="Kernel startup timeout in seconds")
    agent_id: Optional[str] = Field(default=None, max_length=100, description="Optional agent identifier to isolate CWD")
...
class StopKernelArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook whose kernel should be stopped")
...
class InterruptKernelArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook whose kernel should be interrupted")
...
class RestartKernelArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook whose kernel should be restarted")
...
class RunCellArgs(SecureBaseModel):
    ...
    code_override: Optional[str] = Field(None, max_length=100_000, description="Override cell code (max 100KB)")
    task_id_override: Optional[str] = Field(None, max_length=100, description="Client-generated execution ID")
...
class InstallPackageArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook")
    package: str = Field(..., max_length=500, description="Package name or pip specifier")
...
class InspectVariableArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook")
    variable_name: str = Field(..., max_length=200, description="Variable name to inspect")
...
class DetectSyncNeededArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook")
    buffer_hashes: Optional[Dict[int, str]] = Field(default=None, description="Client-side cell hashes")
...
class SyncStateFromDiskArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook")
    strategy: str = Field(default="smart", pattern="^(smart|force|minimal)$", description="Sync strategy")
...
class QueryDataframesArgs(SecureBaseModel):
    ...
    def validate_sql(cls, v):
        ...
        dangerous = ['DROP', 'DELETE', 'TRUNCATE', 'ALTER', 'CREATE TABLE', 'INSERT', 'UPDATE']
        query_upper = v.upper()
...
class SaveCheckpointArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook")
    checkpoint_name: str = Field(default="auto", max_length=100, description="Checkpoint name")
    ...
    def validate_checkpoint_name(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError("Checkpoint name must be alphanumeric with underscores/dashes")
        return v
...
class SwitchKernelEnvironmentArgs(SecureBaseModel):
    ...
    notebook_path: str = Field(..., description="Path to notebook")
    venv_path: str = Field(..., max_length=4096, description="Path to virtual environment")
