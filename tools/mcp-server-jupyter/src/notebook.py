...
async def read_notebook_async(path: str):
    ...
    return await loop.run_in_executor(
        _notebook_io_pool, lambda: nbformat.read(path, as_version=4)
    )
...
def _slice_text(text: str, line_range: Optional[List[int]] = None) -> str:
    ...
    if start < 0:
        start += total_lines
    if end < 0:
        end += total_lines + 1  # +1 because slice is exclusive
    ...
    if target in ["output", "both"] and cell.cell_type == "code":
        ...
        if raw_output:
            ...
            if fmt == "summary":
                ...
                if (
                    len(lines) > 20
                ):  # Slightly larger buffer than 10 to make it worth truncating
                    truncated = (
                        lines[:5]
...
def create_notebook(
    ...
    nb.metadata["language_info"] = {
        ...
        "codemirror_mode": (
            {"name": "ipython", "version": int(python_version.split(".")[0])}
            if language == "python"
            else language
        ),
...
def get_notebook_outline(notebook_path: str) -> List[Dict[str, Any]]:
    ...
    if was_modified:
        ...
        _atomic_write_notebook(nb, path)
        logger.info(
            f"Migrated {notebook_path} to nbformat 4.5 with {cells_updated} Cell IDs"
        )
    ...
    for i, cell in enumerate(nb.cells):
        source_preview = (
            cell.source[:50] + "..." if len(cell.source) > 50 else cell.source
        )
        state = (
            "executed"
        ...
        outline.append(
            {
                ...
                "type": item.get(
                    "cell_type", item.get("kind", "code")
                ),  # VSCode uses 'kind', nbformat uses 'cell_type'
                "source_preview": source_preview.replace("\n", "\\n"),
                "state": state,
            }
        )
...
def insert_cell(
    notebook_path: str, index: int, content: str, cell_type: str = "code"
) -> str:
    """
...
def copy_cell(
    notebook_path: str, index: int, target_index: Optional[int] = None
) -> str:
    ...
    if target_index is None:
        ...
        if target_index < 0:
            ...
            )  # +1 because we're inserting after copying
...
def merge_cells(
    notebook_path: str, start_index: int, end_index: int, separator: str = "\n\n"
) -> str:
...
def change_cell_type(notebook_path: str, index: int, new_type: str) -> str:
    ...
    if new_type not in ["code", "markdown", "raw"]:
        raise ValueError(
            f"Invalid cell type: {new_type}. Must be 'code', 'markdown', or 'raw'"
        )
...
def set_notebook_metadata(notebook_path: str, metadata: Dict[str, Any]) -> str:
    ...
    return "Notebook metadata updated"
