...
class UploadFileRequest(BaseModel):
    target_path: str = Field(..., description="Path where the file should be saved (relative to current working directory)")
...
def _validate_path(path_str: str) -> Path:
    ...
    raise ValueError(f"Access denied: Path {path_str} is outside allowed directories ({cwd})")
...
def register_filesystem_tools(mcp: FastMCP):
    ...
    def upload_file(target_path: str, base64_content: str) -> str:
        ...
        try:
            ...
            try:
                ...
                return ToolResult(success=False, data="", error_msg="Invalid base64 content").to_json()
    ...
    def download_file(source_path: str) -> str:
        ...
        try:
            ...
            if not path.exists():
                return ToolResult(success=False, data="", error_msg=f"File not found: {source_path}").to_json()
            
            if not path.is_file():
                return ToolResult(success=False, data="", error_msg=f"Not a file: {source_path}").to_json()
