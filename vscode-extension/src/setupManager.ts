...
export class SetupManager {
  ...
  private pythonExeForVenv(venvPath: string): string {
    // [FIX: WINDOWS VENV PATHS]
    const isWin = process.platform === 'win32';
    const winPath = path.join(venvPath, 'Scripts', 'python.exe');
    const posixPath = path.join(venvPath, 'bin', 'python');

    // 1. Check strict platform standard first
    if (isWin && fs.existsSync(winPath)) return winPath;
    if (!isWin && fs.existsSync(posixPath)) return posixPath;

    // 2. Fallback check (e.g., using Git Bash on Windows or non-standard venv)
    if (fs.existsSync(winPath)) return winPath;
    if (fs.existsSync(posixPath)) return posixPath;

    // 3. Default based on platform if neither exists yet (creation time)
    return isWin ? winPath : posixPath;
  }
  ...
  async installDependencies(venvPath: string): Promise<void> {
    const pythonExe = this.pythonExeForVenv(venvPath);
    const pythonDistDir = path.join(this.context.extensionPath, 'python_dist');
    const wheelsDir = path.join(this.context.extensionPath, 'wheels');
    
    // Look for the built wheel in python_dist/
    let wheelPath: string | null = null;
    if (fs.existsSync(pythonDistDir)) {
      const files = fs.readdirSync(pythonDistDir);
      const whlFile = files.find(f => f.endsWith('.whl'));
      if (whlFile) {
        wheelPath = path.join(pythonDistDir, whlFile);
      }
    }

    if (!wheelPath) {
      throw new Error('Python server wheel (.whl) not found. Please rebuild the extension with: npm run bundle-python');
    }
    ...
    await vscode.window.withProgress({
      ...
      try {
        ...
        if (hasLocalWheels) {
          ...
          await this.runPipCommand(pythonExe, ['install', wheelPath, '--quiet']);
        }
    ...
    if (result.error) {
      ...
      if (choice === 'Show Details') {
        ...
        terminal.sendText(`"${pythonExe}" -m pip install "${wheelPath}" --verbose`);
  ...
  private runPipCommand(pythonExe: string, args: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
      // [DAY 2 OPT 2.1] Setup 2-minute timeout to prevent indefinite hangs on bad networks
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Pip operation timed out after ${timeoutMs / 1000}s. Check your internet connection or run "mcp-jupyter: Show Server Logs" for details.`));
      }, timeoutMs);

      proc = spawn(pythonExe, ['-m', 'pip', ...args], {
      ...
      proc.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          // Enhance error message with actionable feedback
          const msg = stderr || stdout;
          const preview = msg.substring(0, 200);
          reject(new Error(`pip failed (code ${code}): ${preview}${msg.length > 200 ? '...' : ''}`));
        }
      });
