...
import { checkPythonDependencies } from './dependencies';
...
export async function activate(context: vscode.ExtensionContext): Promise<ExtensionApi | void> {
  ...
  try {
    ...
    mcpClient.autoRestart = config.get<boolean>('autoRestart', true);
    ...
    if (!depsInstalled) {
      ...
      if (configuredPath) {
        try {
          ...
          mcpClient.getOutputChannel().appendLine(`Debug: failed to start server: ${errorMessage}`);
          vscode.window.showWarningMessage(`Failed to start MCP server with configured Python: ${errorMessage}. Open the Setup Wizard for troubleshooting.`);
        }
      }
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          try {
            ...
            mcpClient.getOutputChannel().appendLine(`Activation: mcpClient.start() failed: ${errorMessage}`);
            vscode.window.showErrorMessage(`Failed to start MCP server: ${errorMessage}. Use the Setup Wizard to install or configure the server.`);
          }
        }
      );
    } else {
    ...
    context.subscriptions.push(
      vscode.workspace.onDidSaveNotebookDocument((notebook) => {
        ...
        mcpClient.pruneUnusedAssets(notebook.uri.fsPath, false)
          ...
          .catch((e: unknown) => console.error('GC failed:', e));
      })
    );
    ...
    context.subscriptions.push(
      vscode.workspace.onWillSaveNotebookDocument(event => {
        ...
        if (event.notebook.metadata.mcp_trace || event.notebook.metadata.mcp_execution || event.notebook.metadata.mcp_session) {
          const newMetadata = { ...event.notebook.metadata };
          delete newMetadata.mcp_trace;
          delete newMetadata.mcp_execution;
          // Remove session-level ephemeral state to avoid git churn
          delete newMetadata.mcp_session;
        ...
        event.notebook.getCells().forEach(cell => {
          if (cell.metadata.mcp_trace || cell.metadata.mcp_execution || cell.metadata.execution_timestamp || cell.metadata.agent_id) {
            const newMetadata = { ...cell.metadata };
            delete newMetadata.mcp_trace;
            // Remove execution metadata and noisy timestamps/agent ids
            delete newMetadata.mcp_execution;
            delete newMetadata.execution_timestamp;
            delete newMetadata.agent_id;
      ...
      vscode.commands.registerCommand('mcp-jupyter.selectRuntime', async () => {
        ...
        if (pick === 'Managed Environment') {
          try {
            ...
            const errorMessage = e instanceof Error ? e.message : String(e);
            vscode.window.showErrorMessage(`Failed to create managed environment: ${errorMessage}`);
          }
        } else {
      ...
      vscode.commands.registerCommand('mcp-jupyter.installServer', async () => {
        try {
          ...
          try {
            ...
            const errorMessage = startErr instanceof Error ? startErr.message : String(startErr);
            vscode.window.showWarningMessage(`Installed but failed to start server: ${errorMessage}. Open the Setup Wizard for troubleshooting.`);
          }
        } catch (e: unknown) {
            const errorMessage = e instanceof Error ? e.message : String(e);
          vscode.window.showErrorMessage(`Install failed: ${errorMessage}`);
        }
      })
    );
        ...
        try {
          await vscode.window.withProgress({ title: 'Testing MCP server connection...', location: vscode.ProgressLocation.Notification }, async () => {
            ...
            if (mcpClient.getStatus() !== 'stopped') {
              try { await mcpClient.stop(); } catch { /* Ignore error */ }
            }
            await mcpClient.start();
          });
